...
static int getPeerList_callback ( void *data, int argc, char **argv, char **azColName ) {
    ...
    DB_FOREACH_COLUMN {
        if ( DB_COLUMN_IS ( "id" ) ) {
            if ( strlen ( DB_COLUMN_VALUE ) >= ACPP_PEER_ID_LENGTH ) {
                printde ( "id length exceded (no more %d expected)\n", ACPP_PEER_ID_LENGTH-1 );
                return EXIT_FAILURE;
            }
            strncpy ( list->item[list->length].id, DB_COLUMN_VALUE,ACPP_PEER_ID_LENGTH );
            c++;
        } else if ( DB_COLUMN_IS ( "port" ) ) {
            list->item[list->length].port = DB_CVI;
            c++;
        } else if ( DB_COLUMN_IS ( "ip_addr" ) ) {
            if ( strlen ( DB_COLUMN_VALUE ) >= ACPP_PEER_ADDR_STR_LENGTH ) {
                printde ( "id length exceded (no more %d expected)\n", ACPP_PEER_ADDR_STR_LENGTH-1 );
                return EXIT_FAILURE;
            }
            strncpy ( list->item[list->length].addr_str, DB_COLUMN_VALUE,ACPP_PEER_ADDR_STR_LENGTH );
            c++;
        } else {
