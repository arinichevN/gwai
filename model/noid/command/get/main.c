...
void ngc_free(NoidGetCommand *item){
	mutex_free ( &item->mutex );
}

int ngcList_init(NoidGetCommandList *list, const char *dir, const char *file_name, const char *file_type){
	RESET_LIST(list)
	char path[LINE_SIZE];
	path[0] = '\0';
...
void ngc_sendDataToClient(NoidGetCommand *item, int tcp_fd ) {
...
void ngc_setData(NoidGetCommand *item, const char *data, size_t data_len, int result){
	mutex_lock(&item->mutex);
